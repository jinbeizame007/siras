...
extern crate siras;
use siras::filter_design::{butter, digital_to_analog_cutoff, FilterType};
...
fn main() {
    ...
    let cutoff_frequency_high_pass = digital_to_analog_cutoff(20.0, sample_frequency as f64);
    let cutoff_frequency_low_pass = digital_to_analog_cutoff(20.0, sample_frequency as f64);

    let alpha = 0.5;
    let signal_with_high_pass_filter =
        butter(order, cutoff_frequency_high_pass, FilterType::HighPass)
            .to_discrete(1.0 / sample_frequency as f64, alpha)
            .filtfilt(&x, &t);
    let signal_with_low_pass_filter = butter(order, cutoff_frequency_low_pass, FilterType::LowPass)
        .to_discrete(1.0 / sample_frequency as f64, alpha)
        .filtfilt(&x, &t);
